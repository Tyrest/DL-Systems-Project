...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        attention_scores = self.matmul(q, k) / np.sqrt(q_dim)
        if self.causal:
            causal_mask = self.create_causal_mask(queries_len, keys_values_len, q.device)
            attention_scores = attention_scores + causal_mask.broadcast_to(attention_scores.shape)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        k = self.prenorm_k(k)
        v = self.prenorm_v(v)
        q = q.reshape((batch_size * queries_len, self.q_features))
        k = k.reshape((batch_size * keys_values_len, self.k_features))
        v = v.reshape((batch_size * keys_values_len, self.v_features))
        ...
        attn_output = attn_output.transpose((1, 2)).reshape((batch_size, queries_len, self.num_head * self.dim_head))
