...
devices = ndl.all_devices()


@pytest.mark.parametrize("device", devices)
def test_quantize_round_trip(device):
    arr = np.random.uniform(-2.5, 2.5, size=(4, 5)).astype(np.float32)
    qt = ndl.quantization.quantize_int8(arr)
    recovered = ndl.quantization.dequantize_int8(qt, device=device).numpy()
    np.testing.assert_allclose(recovered, arr, rtol=1e-2, atol=5e-2)


@pytest.mark.parametrize("device", devices)
def test_per_channel_quantization_shapes(device):
...
@pytest.mark.parametrize("device", devices)
def test_linear_quantized_matches_float(device):
    np.random.seed(0)
    lin = ndl.nn.Linear(8, 5, bias=True, device=device)
    x = ndl.Tensor(np.random.randn(4, 8).astype(np.float32), requires_grad=False, device=device)
...
@pytest.mark.parametrize("device", devices)
def test_quantization_per_axis_negative_axis(device):
...
@pytest.mark.parametrize("device", devices)
def test_quantized_tensor_cache_reuse(device):
    arr = np.random.randn(2, 2).astype(np.float32)
    qt = ndl.quantization.quantize_int8(arr)
    first = qt.dequantize(device=device).numpy()
    assert qt.cached_dequantized is not None
    second = qt.dequantize(device=device).numpy()
    np.testing.assert_array_equal(first, second)


@pytest.mark.parametrize("device", devices)
def test_quantized_matmul_shape(device):
    a = ndl.Tensor(np.random.randn(3, 4).astype(np.float32), requires_grad=False, device=device)
...
@pytest.mark.parametrize("device", devices)
def test_linear_disable_quantization(device):
    lin = ndl.nn.Linear(4, 3, bias=False, device=device)
    lin.enable_quantization(axis=1)
    assert lin.use_int8
...
@pytest.mark.parametrize("device", devices)
def test_quantization_bias_effect(device):
    lin = ndl.nn.Linear(3, 2, bias=True, device=device)
    x = ndl.Tensor(np.ones((1, 3), dtype=np.float32), requires_grad=False, device=device)
    float_out = lin(x).numpy()
    lin.eval()
    lin.enable_quantization(axis=1)
    quant_out = lin(x).numpy()
...
@pytest.mark.parametrize("device", devices)
def test_linear_multiple_forward_reuses_quantized(device):
    lin = ndl.nn.Linear(6, 4, bias=True, device=device)
    x = ndl.Tensor(np.random.randn(5, 6).astype(np.float32), requires_grad=False, device=device)
    lin.eval()
    lin.enable_quantization(axis=1)
    out1 = lin(x).numpy()
    out2 = lin(x).numpy()
...
@pytest.mark.parametrize("device", devices)
def test_quantize_asymmetric_roundtrip(device):
...
@pytest.mark.parametrize("device", devices)
def test_quantized_matmul_matches_float(device):
    a = np.random.randn(2, 3).astype(np.float32)
    b = np.random.randn(3, 4).astype(np.float32)
    qt = ndl.quantization.quantize_int8(b, axis=0)
    out = ndl.quantization.quantized_matmul(ndl.Tensor(a, requires_grad=False, device=device), qt).numpy()
...
@pytest.mark.parametrize("device", devices)
def test_linear_train_mode_after_quantization_uses_float(device):
    lin = ndl.nn.Linear(5, 4, bias=True, device=device)
    x = ndl.Tensor(np.random.randn(3, 5).astype(np.float32), requires_grad=False, device=device)
    ref = lin(x).numpy()
    lin.enable_quantization(axis=1)
    lin.train()
    out = lin(x).numpy()
...
@pytest.mark.parametrize("device", devices)
def test_linear_eval_quantization_caches_weight(device):
    lin = ndl.nn.Linear(4, 3, bias=False, device=device)
    lin.eval()
    lin.enable_quantization(axis=1)
    assert lin.use_int8
...
@pytest.mark.parametrize("device", devices)
def test_quantize_small_range(device):
    arr = np.full((3, 3), 1e-4, dtype=np.float32)
    qt = ndl.quantization.quantize_int8(arr)
    rec = ndl.quantization.dequantize_int8(qt, device=device).numpy()
...
@pytest.mark.parametrize("device", devices)
def test_dequantize_reuses_cached_array(device):
    arr = np.random.randn(2, 2).astype(np.float32)
    qt = ndl.quantization.quantize_int8(arr)
    t1 = qt.dequantize(device=device)
    t2 = qt.dequantize(device=device)
